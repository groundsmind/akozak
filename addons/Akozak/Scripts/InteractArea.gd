...
signal interaction_available(interactions: Array[InteractAction])
...
@export var hint_marker: Node
var base_area: BaseArea
var parent_area
var parent_is_3d: bool

func _ready() -> void:
	set_process_unhandled_input(false)
	GlobalInteractData.interaction_finish.connect(_on_global_interaction_finish)
	parent_area = get_parent()
	base_area = BaseArea.new(parent_area)
	base_area.connect_area_entered(_on_area_entered)
	base_area.connect_area_exited(_on_area_exited)
...
func _on_area_entered(_area) -> void:
	...
	if overlap_count == 1:
		...
		interaction_available.emit(interactions)
